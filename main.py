...
# 🛡️ SÉCURITÉ 1 : PROTECTION ANTI-SPAM (Rate Limiting)
# Stocke le timestamp de la dernière requête par user_id
user_last_request_time = {}
ANTI_SPAM_DELAY = 3.0  # Délai minimum de 3 secondes entre 2 requêtes


def check_rate_limit(user_id):
    """Vérifie si l'utilisateur respecte le délai anti-spam.

    Retourne True si l'action est autorisée, False si c'est trop rapide.
    """
    now = time.time()
...
def obtenir_analyses_matchs():
    ...
    try:
        ...
        for idx, match in enumerate(matches, 1):
            ...
            message += (
                f"💡 *Pronostic :* Plus de 1.5 buts / Victoire ou Nul"
                f" {equipe_dom}\n\n"
            )
        ...
        return (
            "⚠️ Erreur lors de la récupération des données de l'API Football."
        )
...
def send_welcome(msg):
    text = (
        ...
        "Cliquez sur la boîte de **Menu** en bas à gauche pour découvrir toutes"
        " les fonctionnalités !"
    )
...
def command_crypto(msg):
    if not check_rate_limit(msg.from_user.id):
        return bot.reply_to(
            msg, "⏳ *Anti-Spam :* Veuillez attendre 3 secondes entre deux actions."
        )
...
def handle_query(call):
    ...
    if not check_rate_limit(user_id):
        bot.answer_callback_query(
            call.id, "⏳ Anti-Spam : Patientez 3 secondes...", show_alert=True
        )
        return
    ...
    if call.data.startswith("c_"):
        ...
        if pair == "custom":
            ...
            bot.send_message(
                chat_id,
                "🔍 Tape le nom de la paire à analyser (ex: `PEPEUSDT`,"
                " `ADAUSDT`) :",
            )
        else:
        ...
        if pair == "custom":
            ...
            bot.send_message(
                chat_id,
                "🔍 Tape le nom de la paire pour le backtest (ex:"
                " `DOGEUSDT`, `LINKUSDT`) :",
            )
        else:
...
def handle_text_messages(msg):
    ...
    if state == "WAITING_BANKROLL":
        if not check_rate_limit(user_id):
            return bot.reply_to(
                msg, "⏳ *Anti-Spam :* Veuillez attendre 3 secondes."
            )
        ...
        try:
            ...
            bot.send_message(
                chat_id,
                "❌ Veuillez entrer un nombre valide (ex: `100000`). Tapez"
                " `/bankroll` pour recommencer.",
            )
