...
def check_and_consume_request_atomic(user_id: int) -> Tuple[bool, str]:
    ...
    with get_db_connection() as conn:
        ...
        cursor.execute(
            "BEGIN IMMEDIATE"
        )  # Verrouille la ligne pour empêcher les accès concourants
...
def command_affiliation(msg):
    ...
    for key, data in AFFILIATES.items():
        markup.add(
            InlineKeyboardButton(
                f"🎁 Partenaire {data['name']}", callback_data=f"aff_{key}"
            )
        )
...
def process_pre_checkout(query: PreCheckoutQuery):
    ...
    if payload.startswith("vip_sub_"):
        if total_amount != 250:
            logger.warning(
                f"PreCheckout Rejeté : Montant VIP incorrect ({total_amount})"
            )
            bot.answer_pre_checkout_query(
                query.id,
                ok=False,
    ...
    with get_db_connection() as conn:
        ...
        if payload.startswith("vip_sub_"):
            if currency == "XTR" and amount == 250:
                ...
                logger.info(
                    f"VIP Activé pour l'utilisateur {user_id} (Tx: {payment_id})"
                )
    ...
    if state == "WAITING_BANKROLL":
        ...
        try:
            ...
            if capital <= 0 or capital > 100_000_000 or capital != capital:
                bot.send_message(
                    chat_id,
                    "❌ **Montant invalide.** Le capital doit être un nombre positif raisonnable. (Aucune requête consommée).",
                )
                return
            ...
            bot.send_message(
                chat_id,
                "❌ Nombre invalide. Entrez un chiffre pur (ex: 5000). (Aucune requête consommée).",
            )
